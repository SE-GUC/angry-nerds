...
AdminDeleteCase: async (req, res) => {
    ...
    SendAttachmentMail: async function (req, res) {
        const email = 'monica.achraff@gmail.com'
          ...
          //      console.log(error)
           // }
            ...
            var attachments = [{
                ...
                content: new Buffer( c.pdfString,'base64'),
                contentType: 'application/pdf'
            }]

            console.log('before')
            await mailer.sendEmail(config.user,'monica.achraff@gmail.com', 'Please verify your email', html, attachments)
            console.log('after')
    ...
    AdminCreateFormType: async function (req,res){
        ...
        try{
            console.log('HHEHHEHEHHE?????   ', req.body)
            const form = FormType.find({formName: req.body.formName})
            //console.log(form)
            // if(form.length !== 0)
